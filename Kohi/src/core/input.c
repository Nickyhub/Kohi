...
void input_system_initialize(u64* memory_requirement, void* state) {
	*memory_requirement = sizeof(input_state);
	...
	EN_INFO("Input subsystem state_ptr.");
}
...
void input_process_key(keys key, b8 pressed) {
	if (key == KEY_LALT) {
		EN_INFO("Left alt pressed.");
	}
	else if (key == KEY_RALT) {
		EN_INFO("Right alt pressed.");
	}
	else if (key == KEY_RCONTROL) {
		EN_INFO("Right ctrl pressed.");
	}
	else if (key == KEY_LCONTROL) {
		EN_INFO("Left ctrl pressed.");
	}
	else if (key == KEY_LSHIFT) {
		EN_INFO("Left shift pressed.");
	}
	else if (key == KEY_RSHIFT) {
		EN_INFO("Right shift pressed.");
	}
