...
b8 application_create(game* game_inst) {
    ...
    bmp_font_config.name = "Font";
    bmp_font_config.resource_name = "Font";
    ...
    if (!ui_text_create(UI_TEXT_TYPE_BITMAP, "Font", 21, "Some test test 123, \n\t yo!", &app_state->test_text)) {
...
b8 application_run() {
    ...
    while (app_state->is_running) {
        ...
        if (!app_state->is_suspended) {
            ...
            // ui
            // Update the bitmap text with camera position. Just using the defualt camera for now.
            ...
            string_format(text_buffer, "Camera Pos: [%.3f, %.3f, %.3f]\nCamera Rot: [%.3f, %.3f, %.3f]",
                pos.x, pos.y, pos.z,
