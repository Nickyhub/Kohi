...
b8 material_system_initialize(u64* memory_requirement, void* state, material_system_config config) {
	...
	*memory_requirement = struct_requirement + array_requirement + hashtable_requirement;
	...
	void* hashtable_block = (void*)((u8*)array_block + array_requirement);
...
material* material_system_acquire(const char* name) {
	...
	if (!resource_system_load(name, RESOURCE_TYPE_MATERIAL, &material_resource)) {
		EN_ERROR("Failed to load material resource, returning nullptr.");
		return 0;
	}
...
material* material_system_acquire_from_config(material_config config) {
	...
	if (state_ptr && hashtable_get(&state_ptr->registered_material_table, config.name, &ref)) {
		...
		if (ref.handle == INVALID_ID) {
			// This means no material exists here. Find a free index fist.
			u32 count = state_ptr->config.max_material_count;
			material* m = 0;
			for (u32 i = 0; i < count; ++i)
			{
				if (state_ptr->registered_materials[i].id == INVALID_ID) {
					// A free slot has beend found. Use its index as the handle.
					ref.handle = i;
			...
			EN_TRACE("Material '%s' does already exists, ref_count increased to %i.", config.name, ref.reference_count);
		}
		// Updated the entry
	...
	EN_ERROR("material_system_acquire_from_config failed to acuire materal '%s'. Null pointer will be returned.", config.name);
	return 0;
}

void material_system_release(const char* name) {
	// Ifnore release requests for the default material.
...
void destroy_material(material* m) {
	...
	//Release the texture references.
	if (m->diffuse_map.texture) {
...
b8 create_default_material(material_system_state* state) {
	...
	if (!renderer_create_material(&state->default_material)) {
		EN_FATAL("Failed to acquire renderer resources for default material. Application cannot continue.");
		return false;
	}
	return true;
}