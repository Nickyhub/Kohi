...
b8 texture_system_initalize(u64* memory_requirement, void* state, texture_system_config config) {
	if (config.max_texture_count == 0) {
		EN_FATAL("texture_system_initialize - config.max_texture_count must be > 0");
		return false;
	}

	//block of memory will contain state structure, then block for array, then block for hastable.
	...
	*memory_requirement = struct_requirement + array_requirement + hashtable_requirement;
	...
	//POTENZIELL FALSCHES CASTING. GIBT LOCKER EINEN FEHLER LOL
	//The array block is after the state. Already allocated, so just set the pointer.
	void* array_block = (void*)((texture*)state + struct_requirement);

	state_ptr->registered_textures = array_block;

	//Hashtable block is after the array.
	void* hashtable_block = (void*)((texture_reference*)array_block + array_requirement);

	//Create hashtable for texture lookups.
	...
	//Fill the hastable with invalid references to use as a default.
...
texture* texture_system_acquire(const char* name, b8 auto_release) {
	//Return default texturte, but warn about it since this should be retuned via get_default_texture();
	if (strings_equali(name, DEFAULT_TEXTURE_NAME)) {
		EN_WARN("texture_system_acquire called for default tesxture. Use texture_system_get_default_texture for texture 'default'.");
	...
	if (state_ptr && hashtable_get(&state_ptr->registered_texture_table, name, &ref)) {
		...
		if (ref.handle == INVALID_ID) {
			...
			t->id = ref.handle;
			EN_TRACE("Texture '%s' does not yet exist. Created and ref_count is now %i.", name);
		}
		else {
			EN_TRACE("Texture '%s' already exists, ref_count increased to %i", ref.reference_count);
		}
	...
	EN_ERROR("texture_system_acquire failed to acquire texture '%s'. Null pointer will be returned.");
	return 0;
}


void texture_system_release(const char* name) {
	//Ignore release requests for the default textures.
	if (strings_equali(name, DEFAULT_TEXTURE_NAME)) {
		return;
	}
	...
	if (state_ptr && hashtable_get(&state_ptr->registered_texture_table, name, &ref)) {
		...
		if (ref.reference_count == 0 && ref.auto_release) {
			...
			//Reset the aray entry, ensure invalid ids are set.
			...
			EN_TRACE("Released texture '%s', now has a reference cout of 'i' (autos_release = '%s'", name, ref.reference_count, ref.auto_release ? "true" : "false");
		}
...
texture* texture_system_get_default_texture() {
	...
	EN_ERROR("get_default_texture called before texture system initialization! Null pointer retuned.");
	return 0;
}
...
b8 load_texture(const char* texture_name, texture* t) {
	...
	//Use a temporary local texture to load into.
	...
	if (data) {
		...
		t->generation = INVALID_ID;
		u64 total_size = (u64)temp_texture.width * temp_texture.height * required_channel_count;

		//Check for transparency
		b32 has_transparency = false;
		for (u64 i = 0; i < total_size; i += required_channel_count) {
			//If the Alpha value is anything less than 255 it has transparency
			u8 a = data[i + 3];
			if (a < 255) {
