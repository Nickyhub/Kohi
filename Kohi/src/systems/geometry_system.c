...
b8 geometry_system_initialize(u64* memory_requirement, void* state, geometry_system_config config) {
	...
	// block of memoery will contain state structure, then block for array, then block for hashtable.
	...
	*memory_requirement = struct_requirement + array_requirement;
	...
	void* array_block = (void*)((u8*)state + struct_requirement);
	...
	if (!create_default_geometries(state_ptr)) {
		EN_FATAL("Failed to create default geometries. Application cannot continue.");
		return false;
	}
	return true;
}
...
void geometry_system_shutdown(void* state) {
	// NOTE: nothing to do here. Vulkan backend handles this by releasing a buffer.
}
...
geometry* geometry_system_acquire_from_config(geometry_config config, b8 auto_release) {
	...
	if (!g) {
		EN_ERROR("Unable to obtain free slot for gemoetry. Adjust configuration to allow more space. Returning nullptr.");
		return 0;
	}
...
void geometry_system_release(geometry* geometry) {
	if (geometry && geometry->id != INVALID_ID) {
		...
		if (ref->geometry.id == geometry->id) {
			...
			// Also blanks out the gemoetry id.
			...
			EN_FATAL("Gemoetry id mismatch. Check registration logic, as this should never occur.");
		}
		return;
	}

	EN_WARN("geometry_system_release cannot release invalid gemoetry id. Nothing was done.");
}
...
geometry* geomtery_system_get_default_2d() {
	...
	EN_FATAL("geometry_system_get_default_2d called before system was initialized. Returning nullptr.");
	return 0;
}
...
b8 create_default_geometries(geometry_system_state* state) {
	...
	// Send the geomtry off to the renderer to be uploaded to the GPU.
	if (!renderer_create_geometry(&state->default_2d_geometry, sizeof(vertex_2d), 4, verts2d, sizeof(u32), 6, indices2d)) {
		EN_FATAL("Failed to create default 2d geometry. Application cannot continue.");
		return false;
	}
...
geometry_config geometry_system_generate_plane_config(f32 width, f32 height, u32 x_segment_count, u32 y_segment_count, f32 tile_x, f32 tile_y, const char* name, const char* material_name) {
	...
	if (x_segment_count < 1) {
		EN_WARN("x_segment_count must be a positive number. Defaultint to one.");
		x_segment_count = 1;
	}
	if (y_segment_count < 1) {
		EN_WARN("y_segment_count must be a positive number. Defaultint to one.");
		y_segment_count = 1;
	}
	...
	if (name && string_length(name) > 0) {
		...
		string_ncopy(config.name, DEFAULT_MATERIAL_NAME, GEOMETRY_NAME_MAX_LENGTH);
	}
