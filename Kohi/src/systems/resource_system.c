...
b8 resource_system_initialize(u64* memory_requirement, void* state, resource_system_config config) {
	if (config.max_loader_count == 0) {
		EN_FATAL("resource_system_initialize failed because config.max_loader_count == 0");
		return false;
	}
	...
	*memory_requirement = sizeof(resource_system_state) + (sizeof(resource_loader) * config.max_loader_count);
	...
	void* array_block = (void*)((u8*)state + sizeof(resource_system_state));
	...
	EN_INFO("Resource system initialized with base path '%s'.", config.asset_base_path);
	return true;
}
...
b8 resource_system_register_loader(resource_loader loader) {
	if (state_ptr) {
		...
		// Ensure no loaders for the given already exist
		for (u32 i = 0; i < count; ++i) {
			resource_loader* l = &state_ptr->registered_loaders[i];
			if (l->id != INVALID_ID) {
				if (l->type == loader.type) {
					EN_ERROR("resource_system_register_loader - Loader of type %d already exists and will not be registered.", loader.type);
					return false;
				}
				else if (loader.custom_type && string_length(loader.custom_type) > 0 && strings_equali(l->custom_type, loader.custom_type)) {
					EN_ERROR("resource_system_register_loader - Loader of custom type %s already exists and will not be registered.", loader.custom_type);
					return false;
				}
...
b8 resource_system_load(const char* name, resource_type type, resource* out_resource) {
	...
	EN_ERROR("resource_system_load - No loader for type %d was found.", type);
	return false;
}
...
b8 resource_system_load_custom(const char* name, const char* custom_type, resource* out_resource) {
	if (state_ptr && custom_type && string_length(custom_type) > 0) {
		...
		for (u32 i = 0; i < count; ++i) {
			resource_loader* l = &state_ptr->registered_loaders[i];
			if (l->id != INVALID_ID && l->type == RESOURCE_TYPE_CUSTOM && strings_equali(custom_type, l->custom_type)) {
				return load(name, l, out_resource);
			}
		}
	}
	...
	out_resource->loader_id = INVALID_ID;
	EN_ERROR("resource_system_load_custom - No loader for type %s was found.", custom_type);
	return false;
}
...
const char* resource_system_base_path() {
	...
	EN_ERROR("resource_system_base_path called before initialization, returning empty string.");
	return "";
}
