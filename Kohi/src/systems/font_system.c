...
b8 font_system_initialize(u64* memory_requirement, void* memory, font_system_config* config) {
	if (config->max_bitmap_font_count == 0 || config->max_system_font_count == 0) {
		KFATAL("font_system_initialize - config.max_bitmap_font_count and config.maxsystem_font_count must be > 0.");
		return false;
	}
...
b8 font_system_load_system_font(system_font_config* config) {
	KERROR("System fonts are not yet supported.");
	return false;
}

b8 font_system_load_bitmap_font(bitmap_font_config* config) {
	// Make sure a font with this name doesnt already exist.
...
b8 font_system_acquire(const char* font_name, u16 font_size, struct ui_text* text) {
	...
	KERROR("Unrecognzied font type: %d.", text->type);
	return false;
}
...
b8 font_system_verify_atlas(font_data* font, const char* text) {
	if (font->type == FONT_TYPE_BITMAP) {
		// Bitmaps dont need verification since they are already generated.
		return true;
	}

	KERROR("font_system_verfy_atlas failed: Unknown font type.");
	return false;
}

b8 setup_font_data(font_data* font) {
	// Create map resoruces
	...
	// Check for a tab glyph, as there may not always be one exprted. If there is, store its
	// x_advance and just use that. If there is not, then create one based off space * 4
	if (!font->tab_x_advance) {
		...
		if (!font->tab_x_advance) {
			...
			if (!font->tab_x_advance) {
				// If still not there than a space wasnt present either, so just
