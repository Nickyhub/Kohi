...
b8 hashtable_set(hashtable* table, const char* name, void* value) {
    ...
    kcopy_memory((u8*)table->memory + (table->element_size * hash), value, table->element_size);
    return true;
}
...
b8 hashtable_get(hashtable* table, const char* name, void* out_value) {
    ...
    u64 hash = hash_name(name, table->element_count);
    kcopy_memory(out_value, (u8*)table->memory + (table->element_size * hash), table->element_size);
    return true;
}
...
b8 hashtable_fill(hashtable* table, void* value) {
    ...
    for (u32 i = 0; i < table->element_count; ++i) {
        kcopy_memory((u8*)table->memory + (table->element_size * i), value, table->element_size);
    }
