...
void hashtable_create(u64 element_size, u32 element_count, void* memory, b8 is_pointer_type, hashtable* out_hashtable) {
	...
	//Might to require an allocator and allocate this memory instead.
...
b8 hashtable_set(hashtable* table, const char* name, void* value) {
	...
	kcopy_memory((void*)((u8*)table->memory + (table->element_size * hash)), value, table->element_size);
	return true;
}
...
b8 hashtable_get(hashtable* table, const char* name, void* out_value) {
	if (!table || !name || !out_value) {
		EN_WARN("hashtable_set requires table, name and value to exist.");
		return false;
	}
	if (table->is_pointer_type) {
		EN_ERROR("hashtable_get should not be used with tables that have pointer types. Use hashtable_get_ptr instead.");
		return false;
	}
	u64 hash = hash_name(name, table->element_count);
	kcopy_memory(out_value, (void*)((u8*)table->memory + (table->element_size * hash)), table->element_size);
	return true;
...
b8 hashtable_get_ptr(hashtable* table, const char* name, void** out_value) {
	if (!table || !name || !out_value) {
		EN_WARN("hashtable_set_ptr requires table, name and value to exist.");
		return false;
	}
	if (!table->is_pointer_type) {
		EN_ERROR("hashtable_set_ptr should not be used with tables that do not have pointer types. Use hashtable_set instead.");
...
b8 hashtable_fill(hashtable* table, void* value) {
	if (!table || !value) {
		EN_WARN("hashtable_fill requires table, name and value to exist.");
		return false;
	}
	if (table->is_pointer_type) {
	...
	for (u32 i = 0; i < table->element_count; ++i) {
		kcopy_memory((void*)((u8*)table->memory + (table->element_size * i)), value, table->element_size);
	}
	return true;
}