...
b8 dynamic_allocator_create(u64 total_size, u64* memory_requirement, void* memory, dynamic_allocator* out_allocator) {
	if (total_size < 1) {
		EN_ERROR("dynamic_allocator_create cannot have a total_size of 0. Create failed.");
			return false;
	}
	if (!memory_requirement) {
		EN_ERROR("dynamic_allocator_create requires memory_requirement to exist. Create failed.");
		return false;
	}
...
b8 dynamic_allocator_destroy(dynamic_allocator* allocator) {
	...
	EN_WARN("dynamic_allocator_destroy requires a pointer to an allocator. Destroy failed.");
	return false;
}
...
void* dynamic_allocator_allocate(dynamic_allocator* allocator, u64 size) {
	if (allocator && size) {
		...
		if (freelist_allocate_block(&state->list, size, &offset)) {
			...
			void* block = (void*)((u8*)state->memory_block + offset);
			return block;
		}
		else {
			EN_ERROR("dynamic_allocator_allocate no lbocks of memory large enough to allocate from.");
			u64 available = freelist_free_space(&state->list);
			EN_ERROR("Requested size: %llu, total space available: %llu", size, available);
			// TODO: report fragmentation?
			return 0;
		}
	}
	EN_ERROR("dynamic_allocator_allocate requires aw valid allocator and size.");
	return 0;
}

b8 dynamic_allocator_free(dynamic_allocator* allocator, void* block, u64 size) {
	if (!allocator || !block || !size) {
		EN_ERROR("dynamic_allocator_free requires both a valid allocator(0x%ü) and a block (0x%p) to be freed.", allocator, block);
		return false;
	}

	dynamic_allocator_state* state = allocator->memory;
	if (block < state->memory_block || block > (u8*)state->memory_block + state->total_size) {
		void* end_of_block = (void*)((b8*)state->memory_block + state->total_size);
		EN_ERROR("dynamic_allocator_free trying to release block (0x%p) outside of allocator range (0x%p) - (0x%p)", block, state->memory_block, end_of_block);
		return false;
	}
	u64 offset = ((u8*)block - state->memory_block);
	if (!freelist_free_block(&state->list, size, offset)) {
		EN_ERROR("dynamic_allocator_free failed.");
		return false;
	}
	return true;
}
