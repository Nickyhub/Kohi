...
void* linear_allocator_allocate(linear_allocator* allocator, u64 size) {
	if (allocator && allocator->memory) {
		if (allocator->allocated + size > allocator->total_size) {
			...
			EN_ERROR("linear_allocate_allocate - Tried to allocate %lluB, only %llu remaining", size, remaining);
			return 0;
		}
	...
	EN_ERROR("linear_allocator_allocate - provided allocator not initialized.");
	return 0;
}
