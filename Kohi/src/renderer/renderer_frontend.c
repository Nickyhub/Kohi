...
b8 renderer_renderbuffer_bind(renderbuffer* buffer, u64 offset) {
    if (!buffer) {
        KERROR("renderer_renderbuffer bind requires a valid pointer to a buffer.");
        return false;
    }
...
b8 renderer_renderbuffer_allocate(renderbuffer* buffer, u64 size, u64* out_offset) {
    ...
    if (buffer->freelist_memory_requirement == 0) {
        KWARN("vulkan_buffer_allocate called on a buffer not using freelists. Offset will not be valid. Call vulkan_buffer_load_range instead.");
        *out_offset = 0;
        return true;
    }
...
b8 renderer_renderbuffer_free(renderbuffer* buffer, u64 size, u64 offset) {
    if (!buffer || !size) {
        KERROR("vulkan_buffer_free requires valid buffer and nonzero size.");
        return false;
    }

    if (buffer->freelist_memory_requirement == 0) {
        KWARN("vulkan_buffer_allocate called on a buffer not using freelists Notthing was done.");
        return true;
    }
