...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const renderer_backend_config* config, u8* out_window_render_target_count) {
    ...
    const char** required_extension = &VK_KHR_SURFACE_EXTENSION_NAME;
    darray_push(required_extensions, required_extension);  // Generic surface extension
    platform_get_required_extension_names(&required_extensions);       // Platform-specific extension(s)
    u32 required_extension_count = 0;
#if defined(_DEBUG)
    required_extension = &VK_EXT_DEBUG_UTILS_EXTENSION_NAME;
    darray_push(required_extensions, required_extension);  // debug utilities
...
// and make sure they exist. Validation layers should only be enabled on non-release builds.
    ...
    const char** required_validation_layer = &"VK_LAYER_KHRONOS_validation";
    darray_push(required_validation_layer_names, required_validation_layer);
