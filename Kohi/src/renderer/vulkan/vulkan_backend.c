...
void vulkan_alloc_free(void* user_data, void* memory) {
	...
	u64 size;
	u16 alignment;
	b8 result = kmemory_get_size_aligment(memory, &size, &alignment);
...
void* vulkan_alloc_reallocation(void* user_data, void* original, size_t size, size_t alignment, VkSystemAllocationScope allocation_scope) {
	...
	u64 alloc_size;
	u16 alloc_alignment;
	b8 is_aligned = kmemory_get_size_aligment(original, &alloc_size, &alloc_alignment);
	if (!is_aligned) {
...
#ifdef KVULKAN_ALLOCATOR_TRACE
	KTRACE("vulkan_alloc_reallocation of unaligned block %p.", original);
#endif
...
#endif
		// Free original memory only if the new allocation was successful
...
#endif
	kfree_report(size, MEMORY_TAG_VULKAN_EXT);
}
...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const renderer_backend_config* config, u8* out_window_render_target_count) {
	...
	darray_push(required_extensions, extension_name); // debug utilities
	...
	int a = darray_length(available_layers);
	VK_CHECK(vkEnumerateInstanceLayerProperties(&available_layer_count, available_layers));
	a = darray_length(available_layers);
...
b8 vulkan_renderer_create_geometry(geometry* geometry, u32 vertex_size, u32 vertex_count, const void* vertices, u32 index_size, u32 index_count, const void* indices) {
	...
	if (!renderer_renderbuffer_load_range(&context.object_vertex_buffer, internal_data->vertex_buffer_offset, total_size, vertices)) {
		KERROR("vulakn_renderer_create_geometry failed to upload to the vertex buffer!.");
		return false;
	}
...
void vulkan_renderer_draw_geometry(geometry_render_data* data) {
	...
	b8 indcludes_index_data = buffer_data->index_count > 0;
	if (!vulkan_buffer_draw(&context.object_vertex_buffer, buffer_data->vertex_buffer_offset, buffer_data->vertex_count, indcludes_index_data)) {
		KERROR("vulkan_renderer_draw_geometry failed to draw vertex buffer.");
		return;
	}

	if (indcludes_index_data) {
...
// Indicates if the providedc buffer has host-visible memory.
...
b8 vulkan_buffer_create_internal(renderbuffer* buffer) {
	...
	// Allocate the internal state block of memory at the end once we are sure verything was created successfully.
...
b8 vulkan_buffer_bind(renderbuffer* buffer, u64 offset) {
	if (!buffer || !buffer->internal_data) {
		KERROR("vulkan_buffer_bind requires a valid pointer to a buffer.");
		return false;
	}
	vulkan_buffer* internal_buffer = (vulkan_buffer*)buffer->internal_data;
...
b8 vulkan_buffer_flush(renderbuffer* buffer, u64 offset, u64 size) {
	...
	// NOTE if not host-cohere, flush the mapped memory gange.
	vulkan_buffer* internal_buffer = (vulkan_buffer*)buffer->internal_data;
...
b8 vulkan_buffer_read(renderbuffer* buffer, u64 offset, u64 size, void** out_memory) {
	if (!buffer || !buffer->internal_data || !out_memory) {
		KERROR("vulkan_buffer_read requires a vlia dpointer to a buffer and out_memory, and the size must be nonzero.");
		return false;
	}

	vulkan_buffer* internal_buffer = (vulkan_buffer*)buffer->internal_data;
	if (vulkan_buffer_is_device_local(internal_buffer) && !vulkan_buffer_is_host_visible(internal_buffer)) {
		// NOTE If a read buffer is needed the target buffer's memory is not host visible but is device-local,
		// create the read buffer, copy data to it, then read from that buffer.

		// Create a host-visible staging buffer to dopy to. Mark it as the destination of the transfer.
		renderbuffer read;
		...
		// If no staging buffer is neede, map/copy/unmap.
		void* data_ptr;
...
b8 vulkan_buffer_copy_range(renderbuffer* source, u64 source_offset, renderbuffer* dest, u64 dest_offset, u64 size) {
	if (!source || !source->internal_data || !dest || !dest->internal_data || !size) {
		KERROR("vulkan_buffer_copy_range requires a valid pointers to source and destination buffers as well as nonzero size.");
		return false;
	}
