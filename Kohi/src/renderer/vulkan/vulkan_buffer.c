...
b8 vulkan_buffer_create(
	...
	if (out_buffer->memory_index == -1) {
		EN_ERROR("Unable to create vulkan buffer because the required memory type index was not found.");
		return false;
	}
	...
	if (result != VK_SUCCESS) {
		EN_ERROR("Unable to create vulkan buffer because the required memory allocation failed. Error: %i.", result);
		return false;
	}
...
b8 vulkan_buffer_resize(
	...
	if (result != VK_SUCCESS) {
		EN_ERROR("Unable to resize vulkan buffer because the required memory allocation failed. Error: %i", result);
		return false;
	}
...
void vulkan_buffer_copy_to(
	...
	//Submit the buffer for exectuion and wait for it to complete.
