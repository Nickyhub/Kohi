...
b8 vulkan_fence_wait(vulkan_context* context, vulkan_fence* fence, u64 timeout_ns) {
	if (!fence->is_signaled) {
		...
		case VK_SUCCESS:
			...
			return true;
		case VK_TIMEOUT:
			EN_WARN("vk_fence_wait - Timed out");
			break;
		case VK_ERROR_DEVICE_LOST:
			EN_ERROR("vk_fence_wait - VK_ERROR_DEVICE_LOST.");
			break;
		case VK_ERROR_OUT_OF_HOST_MEMORY:
			EN_ERROR("vk_fence_wait - VK_ERROR_OUT_OF_HOST_MEMORY.");
			break;
		case VK_ERROR_OUT_OF_DEVICE_MEMORY:
			EN_ERROR("vk_fence_wait - VK_ERROR_OUT_OF_DEVICE_MEMORY.");
			break;
		default:
			EN_ERROR("vk_fence_wait - An unknown error has occurred");
		}
	}
	else {
		return true;
