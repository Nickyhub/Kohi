...
b8 vulkan_material_shader_create(vulkan_context* context, vulkan_material_shader* out_shader) {
    ...
    for (u32 i = 0; i < MATERIAL_SHADER_STAGE_COUNT; ++i) {
        if (!create_shader_module(context, BUILTIN_SHADER_NAME_MATERIAL, stage_type_strs[i], stage_types[i], i, out_shader->stages)) {
            EN_ERROR("Unable to create %s shader module for '%s'.", stage_type_strs[i], BUILTIN_SHADER_NAME_MATERIAL);
            return false;
        }
    }
    ...
    object_pool_sizes[1].descriptorCount = VULKAN_MATERIAL_SHADER_DESCRIPTOR_COUNT * VULKAN_MAX_MATERIAL_COUNT;
...
#define ATTRIBUTE_COUNT 2
    ...
    if (!vulkan_graphics_pipeline_create(
            ...
            true,
            &out_shader->pipeline)) {
        EN_ERROR("Failed to load graphics pipeline for object shader.");
        return false;
    }
    ...
    if (!vulkan_buffer_create(
        ...
        EN_ERROR("Material instance buffer creation failed for shader.");
        return false;
    }
...
void vulkan_material_shader_apply_material(vulkan_context* context, struct vulkan_material_shader* shader, material* material) {
    if (context && shader) {
        ...
        vulkan_material_shader_instance_state* object_state = &shader->object_states[material->internal_id];
...
void vulkan_material_shader_release_resources(vulkan_context* context, struct vulkan_material_shader* shader, material* material) {
    vulkan_material_shader_instance_state* instance_state = &shader->object_states[material->internal_id];
