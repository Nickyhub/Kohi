...
typedef struct vulkan_physcial_device_queue_family_info {
	u32 graphics_family_index;
	u32 present_family_index;
	u32 compute_family_index;
	u32 transfer_family_index;
} vulkan_physcial_device_queue_family_info;
...
b8 physical_device_meets_requirements(
	...
	vulkan_physcial_device_queue_family_info* out_queue_info,
...
b8 vulkan_device_create(vulkan_context* context) {
	...
	EN_INFO("Creating logical device...");
	...
	VkDeviceQueueCreateInfo queue_create_infos[32] = { 0 };
	...
	EN_INFO("Logical device created.");
	...
	EN_INFO("Graphics command pool created.");
...
b8 select_physical_device(vulkan_context* context) {
	...
	if (physical_device_count == 0) {
		EN_FATAL("No devices which support Vulkan were found.");
		return false;
	}
	...
	for (u32 i = 0; i < physical_device_count; ++i) {
		...
		darray_push(requirements.device_extension_names, extension);

		vulkan_physcial_device_queue_family_info queue_info = { 0 };
		...
		if (result) {
			EN_INFO("Selected device: '%s'.", properties.deviceName);
			// GPU type, etc.
			switch (properties.deviceType) {
			default:
			case VK_PHYSICAL_DEVICE_TYPE_OTHER:
				EN_INFO("GPU type is Unknown.");
				break;
			case VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU:
				EN_INFO("GPU type is Integrated.");
				break;
			case VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU:
				EN_INFO("GPU type is Descrete.");
				break;
			case VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU:
				EN_INFO("GPU type is Virtual.");
				break;
			case VK_PHYSICAL_DEVICE_TYPE_CPU:
				EN_INFO("GPU type is CPU.");
				break;
			}

			EN_INFO("GPU Driver version: %d %d %d",
				VK_VERSION_MAJOR(properties.driverVersion),
				VK_VERSION_MINOR(properties.driverVersion),
				VK_VERSION_PATCH(properties.driverVersion));
			EN_INFO("Vulkan API version: %d %d %d",
	...
	if (!context->device.physical_device) {
		EN_ERROR ("No physical devices were found which meet the requiremtns");
		return false;
	}

	EN_INFO("Physical device selected.");
	return true;
}
...
b8 physical_device_meets_requirements(
	...
	if (requirements->discrete_gpu) {
		if (properties->deviceType != VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
			EN_INFO("Device is not a descrete GPU, and one is required. Skipping.");
			return false;
		}
	}
	...
	EN_INFO("Graphics | Present | Compute | Transfer | Name");
	...
	if (
		...
		EN_TRACE("Graphics Family Index: %i", out_queue_info->graphics_family_index);
		EN_TRACE("Present Family Index:  %i", out_queue_info->present_family_index);
		EN_TRACE("Transfer Family Index: %i", out_queue_info->transfer_family_index);
		EN_TRACE("Compute Family Index:  %i", out_queue_info->compute_family_index);
	}
	...
	if (out_swapchain_support->format_count < 1 || out_swapchain_support->present_mode_count < 1) {
		...
		EN_INFO("Required swapchain support not present, skipping device.");
		return false;
	}
	...
	if (requirements->device_extension_names) {
		...
		if (available_extension_count != 0) {
			...
			for (u32 i = 0; i < required_extension_count; ++i) {
				...
				if (!found) {
					EN_INFO("Required extension not found: '%s', skipping device.", requirements->device_extension_names[i]);
