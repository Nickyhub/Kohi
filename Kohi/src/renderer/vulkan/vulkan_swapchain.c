...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
	...
	//If preferred format was not found then just chose the first one
	if (!found) {
		swapchain->image_format = context->device.swapchain_support.formats[0];
	}

	//default to fifo, if mailbox exists use that one
	...
	swapchain_extent.width = EN_CLAMP(swapchain_extent.width, min.width, max.width);
	swapchain_extent.height = EN_CLAMP(swapchain_extent.height, min.height, max.height);
	...
	if (!vulkan_device_detect_depth_format(&context->device)) {
		...
		EN_FATAL("Failed to find a supported format!");
	}
	...
	EN_INFO("Swapchain created successfully!");
}
