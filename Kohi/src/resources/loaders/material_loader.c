...
b8 material_loader_load(struct resource_loader* self, const char* name, resource* out_resource) {
	...
	if (!filesystem_open(full_file_path, FILE_MODE_READ, false, &f)) {
		EN_ERROR("material_loader_load - unable to open material file for reading: '%s'.", full_file_path);
		return false; 
	}

	// TODO: Shoudl be using an allocator here.
	...
	while (filesystem_read_line(&f, 511, &p, &line_length)) {
		...
		if (equal_index == -1) {
			EN_WARN("Potential formatting issue found in file '%s': '=' token not found. Skipping line %ui.", full_file_path, line_number);
			line_number++;
			continue;
		}
		...
		if (strings_equali(trimmed_var_name, "version")) {
			...
			if (!string_to_vec4(trimmed_value, &resource_data->diffuse_colour)) {
				EN_WARN("Error parsing diffuse_colour in file '%s'. Using default of white instead.", full_file_path);
				// NOTE: White is already assigned above to the colour. No need to do it here.
			}
		}
...
void material_loader_unload(struct resource_loader* self, resource* resource) {
	if(!resource_unload(self, resource, MEMORY_TAG_MATERIAL_INSTANCE)) {
		EN_WARN("material_loader_unload called with nullptr for self or resource.");
	}
}
