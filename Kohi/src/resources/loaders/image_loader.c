...
b8 image_loader_load(struct resource_loader* self, const char* name, resource* out_resource) {
	...
	// TODO: extend this to makt it configurable
	stbi__err(0, 0);
	u8* data = 0;
	data = stbi_load(
		full_file_path,
	...
	if (!data) {
		EN_ERROR("Image resource loader failed to load file '%s' : %s", full_file_path, fail_reason);
	...
	if (!data) {
		EN_ERROR("Image resource loader failed to load file '%s'.", full_file_path);
	}
...
void image_loader_unload(struct resource_loader* self, resource* resource) {
	if (!resource_unload(self, resource, MEMORY_TAG_TEXTURE)) {
		EN_WARN("image_loader_unload called with nullptr for self or resource.");
	}
}
